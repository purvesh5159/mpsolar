...
Vtiger_Edit_Js("Inventory_Edit_Js", {
    ...
    initializeVariables : function() {
        ...
        this.stcTotalEle = jQuery('#stcTotal');
        ...
        this.depositTypeEles = jQuery('input[name="deposit"]');
    ...
    updateRowNumberForRow : function(lineItemRow, expectedSequenceNumber, currentSequenceNumber){
		...
		var idFields = new Array('productName','subproduct_ids','hdnProductId','purchaseCost','margin',
									'comment','assignqty','qty','listPrice','discount_div','discount_type','discount_percentage',
									'discount_amount','lineItemType','searchIcon','netPrice','subprod_names',
									'productTotal','discountTotal','totalAfterDiscount','taxTotal','paymentmode','paymentnumber','bankdetails','paymentdate');
	...
	calculateLineItemTotal : function (lineItemRow) {
		...
		if(module == "Payment"){
			var lineItemTotal = parseFloat(listPrice);
		}
		else
		{
...
//		this.calculateGrandTotal();
    ...
    calculateGrandTotal : function(){
		...
		/*let moduleNameCust = app.getModuleName();
        if( moduleNameCust == 'Quotes'){
             let valOfstc = jQuery('input[name="noofstc"]').val();
             if(valOfstc == '' || valOfstc == undefined || valOfstc == null){
                 valOfstc = 0;
             } else {
                 valOfstc = parseFloat(jQuery('input[name="noofstc"]').val());
             }
             grandTotal = grandTotal - valOfstc;  

			grandTotal = grandTotal.toFixed(this.numOfCurrencyDecimals);
			this.setGrandTotal(grandTotal);

		let deposit =jQuery('input[name="deposit"]').val();
        if (deposit == "") {
            jQuery('input[name="deposit"]').val(0);
            deposit = 0;
        }
    ...
    mapResultsToFields: function(parentRow,responseData){
		...
		for(var id in responseData){
			console.log(responseData[id]['stccal']);
				if(responseData[id]['stccal'] == '1'){
				jQuery('.stcigField').val(id);
				jQuery('.stcTotalVal').val(responseData[id]['sizeinwatts']);
				jQuery('.dpvalue').val(responseData[id]['dpvalue']);
			}
    ...
    registerQuantityChangeEvent : function() {
		...
		this.lineItemsHolder.on('focusout','.qty',function(e){
			...
			if(jQuery('.selectedModuleId', lineItemRow).val() == jQuery('.stcigField').val()){
				...
				console.log(dpvalue);
				let noofstcvalue = (sizeofsystem * dpvalue * 1.382  - 1)/1000 ;
				let noofstc = Math.round(noofstcvalue);
	...
	/**
	 ...
	 registerListPriceChangeEvent : function() {
		...
		jQuery('#lineItemTab').on('focusout', 'input.listPrice',function(e){
			var element = jQuery(e.currentTarget);
...
//						}
    ...
    //purvesh
	registerSTCChange: function () {
        var self = this;
        jQuery('#Quotes_editView_fieldName_noofstc').on('change', function (e) {
            var element = jQuery(e.currentTarget);
		...
		this.depositTypeEles.on('change', function(e){
          console.log('in');
            var element = jQuery(e.currentTarget);
            let deposit = element.val();
            if (deposit == "") {
                element.val("0");
				deposit = 0;
            }
			let balance = parseFloat(self.getGrandTotal()) -  parseFloat(deposit);
			$("input[name='balance']").val(balance);
        });

		jQuery('input[name="stc"]').on('change', function (e) {
            var element = jQuery(e.currentTarget);
            let stc = element.val();
            if (stc == "") {
                element.val("0");
...
//			};
    ...
    registerFieldDependencyEvent: function (container) {
     ...
     registerQuoteFieldDependencyEvent: function (container) {
        var Tasktype = $("select[name='saletype'] option:selected").val();
        jQuery('#Quotes_editView_fieldName_existingsystemdetails').addClass('hide');
        jQuery('#existingsystemdetails').addClass('hide');
        
        if (Tasktype != 'New') {
            jQuery('#Quotes_editView_fieldName_existingsystemdetails').removeClass('hide');
			jQuery('#existingsystemdetails').removeClass('hide');
        }
        jQuery("select[name='saletype']").on('change', function(e){
            var currentTarget = jQuery(e.currentTarget);
            var subTasktype = currentTarget.val();
            if (subTasktype == 'New') {
                jQuery('#Quotes_editView_fieldName_existingsystemdetails').addClass('hide');
    ...
    registerBasicEvents: function(container){
        ...
        this.registerQuoteFieldDependencyEvent();
        this.registerSTCChange();
    },
});
