			...
			<table id="listview-table" class="table {if $LISTVIEW_ENTRIES_COUNT eq '0'}listview-table-norecords {/if} listview-table ">
				...
				<tbody class="overflow-y">
					{foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=listview}
						...
						{/if}
						<tr class="listViewEntries" data-id='{$DATA_ID}' user="{$ASSIGNEDID}" 
                        {if !$CURRENT_USER_MODEL->isAdminUser()} 
						data-recordUrl='{$DATA_URL}&mode=showDetailViewByMode&requestMode=full&app={$SELECTED_MENU_CATEGORY}' {else} data-recordUrl='{$DATA_URL}&app={$SELECTED_MENU_CATEGORY}' {/if} id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}" {if $MODULE eq 'Calendar'}data-recurring-enabled='{$LISTVIEW_ENTRY->isRecurringEnabled()}'{/if}>
							...
							<span class="fieldValue">
								<span class="value">
									{if ($LISTVIEW_HEADER->isNameField() eq true or $LISTVIEW_HEADER->get('uitype') eq '4') and $MODULE_MODEL->isListViewNameFieldNavigationEnabled() eq true }
										...
										{/if}
									{else if $MODULE_MODEL->getName() eq 'Leads' && $LISTVIEW_HEADERNAME eq 'mobile' && !$CURRENT_USER_MODEL->isAdminUser()}
									XXXXXXX{$LISTVIEW_ENTRY_VALUE|substr:($LISTVIEW_ENTRY_VALUE|substr:"|"-3)}
