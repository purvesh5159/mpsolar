...
class Inventory_GetTaxes_Action extends Vtiger_Action_Controller {
	...
	function process(Vtiger_Request $request) {
		...
		$dp = $adb->pquery('select fax from `vtiger_organizationdetails` where organization_id = 1');
		$dpvalue = $adb->query_result($dp,0,'fax');
		...
		foreach($idList as $id) {
			$resultData = array(
								...
								'stccal'          => $recordModel->get('stccal'),
								'sizeinwatts'           => $recordModel->get('sizeinwatts'),
								'dpvalue' => $dpvalue
					);
