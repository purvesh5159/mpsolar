...
function addInventoryHistory($module, $id, $relatedname, $total, $history_fldval)
{
	...
	$history_table_array = Array(
					...
					"Payment"=>"vtiger_paymentstatushistory"
				    );
...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='')
{
	...
	for($i=1; $i<=$tot_no_prod; $i++)
	{
        ...
        $assignqty = vtlib_purify($_REQUEST['assignqty'.$i]);
		...
		if($module == 'Payment')
		{
		$query ="INSERT INTO vtiger_inventoryproductrel(id, productid, sequence_no, quantity, bankdetails, paymentmode, paymentdate, paymentnumber, listprice, comment, description) VALUES(?,?,?,?,?,?,?,?,?,?,?)";
		$qparams = array($focus->id,$prod_id,$prod_seq,$qty,$bankdetails,$paymentmode,$paymentdate,$paymentnumber,$listprice,$comment,$description);
		$adb->pquery($query,$qparams);
	   }
	 elseif($module == 'SalesOrder')
	   {
		$query = 'INSERT INTO vtiger_inventoryproductrel(id, productid, sequence_no, assignqty, quantity, listprice, comment, description, purchase_cost, margin)
					VALUES(?,?,?,?,?,?,?,?,?,?)';
		$qparams = array($focus->id,$prod_id,$prod_seq,$assignqty,$qty,$listprice,$comment,$description, $purchaseCost, $margin);
		$adb->pquery($query,$qparams);
	    }
	    else{
