...
class Maintenance extends Vtiger_CRMEntity {
	...
	var $related_tables = Array( 
					'vtiger_maintenancecf' => Array('maintenanceid')
					);
	...
	var $list_fields = Array (
		...
		'Maintenance No' => Array('maintenance', 'maintenanceno'),
/*'Maintenance No'=> Array('maintenance', 'maintenanceno'),*/
...
/*'Maintenance No'=> 'maintenanceno',*/
	...
	var $list_link_field = 'maintenanceno';
...
/*'Maintenance No'=> 'maintenanceno',*/
	...
	var $popup_fields = Array ('maintenanceno');

	// For Alphabetical search
	var $def_basicsearch_col = 'maintenanceno';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'maintenanceno';

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('maintenanceno','assigned_user_id');

	var $default_order_by = 'maintenanceno';
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {
			...
			Maintenance::checkWebServiceEntry();
			Maintenance::createUserFieldTable($moduleName);
			Maintenance::addInTabMenu($moduleName);
	...
	static function checkWebServiceEntry() {
		...
		$sql       =  "SELECT count(id) AS cnt FROM vtiger_ws_entity WHERE name = 'Maintenance'";
		...
		if($adb->num_rows($result) > 0)
		{
			...
			if($no == 0)
			{
				...
				$ws_entitySql = "INSERT INTO vtiger_ws_entity ( id, name, handler_path, handler_class, ismodule ) VALUES".
						  " (?, 'Maintenance','include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation' , 1)";
