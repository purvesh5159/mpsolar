...
class Vtiger_Detail_View extends Vtiger_Index_View {
	...
	function preProcess(Vtiger_Request $request, $display=true) {
		...
		$recordId = $request->get('record');
		$query = $adb->pquery('select smownerid from vtiger_crmentity where crmid='.$recordId);
		$assignid = $adb->query_result($query,0,'smownerid');
		$moduleName = $request->getModule();
		if(!$this->record){
		...
		$viewer->assign('ASSIGNEDID', $assignid);
		$viewer->assign('CURRENTID', $currentid);
