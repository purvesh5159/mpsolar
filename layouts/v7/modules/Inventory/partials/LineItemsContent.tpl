...
{strip}
    ...
    {assign var="assignqty" value="assignqty"|cat:$row_no}
   ...
   <td>
	...
	{if $MODULE eq 'SalesOrder'}
	     <td>
			<input id="{$assignqty}" name="{$assignqty}" type="text" class="assignqty smallInputBox inputElement"
				   data-rule-required=true data-rule-positive=true data-rule-greater_than_zero=true value="{if !empty($data.$assignqty)}{$data.$assignqty}{else}1{/if}"
				   {if $QUANTITY_EDITABLE eq false} disabled=disabled {/if} />
		</td>
	 {/if}
	{/if}
