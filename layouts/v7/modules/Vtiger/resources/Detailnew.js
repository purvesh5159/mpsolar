...
Vtiger.Class("Vtiger_Detail_Js",{
	...
	*/
	...
	saveFieldValues : function (fieldDetailList) {
		...
		//create project and invoice while status changed to accepeted
		if (fieldDetailList['field'] == "quotestage" && fieldDetailList['value'] == "Accepted") {
			...
			app.helper.showConfirmationBox({ 'message': message }).then(
				function (e) {
					...
					aDeferred1.promise();
					//app.helper.showSuccessNotification({message: app.vtranslate('Sucessfully Record Created')});
				},
				function (error, err) {
					var aDeferred = jQuery.Deferred();
					var recordId = this.getRecordId();
		 ...
		 if (moduleNameOFCust == "SalesOrder"  && fieldDetailList['field'] == "sostatus" && fieldDetailList['value'] == "Approved") {
			let message = "Create Maintance";
			app.helper.showConfirmationBox({ 'message': message }).then(
				function (e) {
					var module = app.getModuleName();
					var aDeferred1 = jQuery.Deferred();
		...
		//add stock to installer inventory while status changed to dispatched
        if (moduleNameOFCust == "SalesOrder"  && fieldDetailList['field'] == "sostatus" && fieldDetailList['value'] == "Dispatched") {
		...
		//send mail to installer to collect stock of inventory
		...
		//update used stock to installer inventory
	...
	registerBasicEvents: function(){
		...
		//console.log(b);
			if (a != b)
			{
				...
				jQuery('.tab-item').css('pointer-events', 'none');
			}
			else if(b == 1){
				jQuery('.block').removeClass('hide'); 
				jQuery('.tab-item').css('pointer-events', '');
			}
	    }
